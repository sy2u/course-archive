...
#define SCAN_BLOCK_WIDTH HISTOGRAM_LENGTH/2
  ...
  __global__ void get_hist( unsigned char* grayImage, float* histogram, int width, int height ){
    int col = threadIdx.x + blockIdx.x * blockDim.x;
    int row = threadIdx.y + blockIdx.y * blockDim.y;
    int image_idx = row * width + col;
    int hist_idx = threadIdx.x + threadIdx.y * blockDim.x;

    __shared__ float private_hist[HISTOGRAM_LENGTH];
  ...
  // Imported from Lab6:
  //---------------------------------------------------------------------------------
  __global__ void scan_tree(float *input, float *output, int len, int image_size) {
    __shared__ float T[2*SCAN_BLOCK_WIDTH];
    ...
    // first pass
    int stride = 1;
    while( stride < 2*SCAN_BLOCK_WIDTH ){
      __syncthreads();
      int idx = (threadIdx.x+1)*stride*2 - 1;
      if(idx < 2*SCAN_BLOCK_WIDTH && (idx-stride) >= 0)
      T[idx] += T[idx-stride];
      stride = stride*2;
    }
    // second pass: post scan
    stride = SCAN_BLOCK_WIDTH/2;
    while(stride > 0) {
      __syncthreads();
      int idx = (threadIdx.x+1)*stride*2 - 1;
      if ((idx+stride) < 2*SCAN_BLOCK_WIDTH)
      T[idx+stride] += T[idx];
      stride = stride / 2;
    }
    // copy back
  ...
  __global__ void scan_getsum(float *input, float *sums, int numblocks) {
    ...
    if( (threadIdx.x == SCAN_BLOCK_WIDTH-1) && (blockIdx.x < numblocks) ){
...
int main(int argc, char **argv) {
  ...
  cudaMalloc((void**)&device_hist, sizeof(float)*HISTOGRAM_LENGTH);
  ...
  get_hist<<<GridDim_Hist,BlockDim_Hist>>>(deviceInput_gray, device_hist, imageWidth, imageHeight);
  ...
  dim3 dimblock(SCAN_BLOCK_WIDTH,1,1);
  scan_tree<<<dimgrid,dimblock>>>(device_hist,device_cdf,HISTOGRAM_LENGTH,flat_size);
