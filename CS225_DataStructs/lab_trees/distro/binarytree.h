...
class BinaryTree
{
  ...
  private:
     ...
     * @param subRoot The current node in the recursion
     */
    void isOrdered(const Node* subRoot, bool& head, T& prev, bool& retval) const;

    /**
     * Private helper function for the public printPaths function.
     * @param subRoot The current node in the recursion
     */
    void printPaths(const Node* subRoot, vector<vector<T> > &paths, vector<T> track) const;

    /**
     * Private helper function for the public sumDistances function.
     * @param subRoot The current node in the recursion
     */
    void sumDistances(const Node* subRoot, int& sum, int curr) const;
