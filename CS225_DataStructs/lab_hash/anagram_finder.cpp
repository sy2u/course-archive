...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
    ...
    for (size_t i = 0; i < word.length(); i++){
        word_hashTable[word[i]]++;
        test_hashTable[test[i]]++;
    }
    // identify if two hash tables are the same
    typename Dict<char, int>::iterator word_it = word_hashTable.begin(); 
    typename Dict<char, int>::iterator test_it = test_hashTable.begin();
    while( word_it!=word_hashTable.end() && test_it!=test_hashTable.end() ){
        if( *word_it != *test_it ){ return false; }
        word_it++; test_it++;
    }
    if( word_it==word_hashTable.end() && test_it==test_hashTable.end() ){ return true; }
    return false;
}
