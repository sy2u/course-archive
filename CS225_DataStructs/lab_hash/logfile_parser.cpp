...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
        ...
        string information = ll.customer+ll.url;
        if( !whenVisitedTable.keyExists(information) ){
            whenVisitedTable[information] = ll.date;
        } else {
            if( whenVisitedTable[information] < ll.date ){
                whenVisitedTable[information] = ll.date;
            }
        }
    }
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
    ...
    string information = customer+url;
    return whenVisitedTable.keyExists(information);
}
                                  ...
                                  const string& url) const
{
    ...
    //(void) url;      // prevent warnings... When you implement this function, remove this line.
    string information = customer+url;
    return whenVisitedTable.find(information); 
}
