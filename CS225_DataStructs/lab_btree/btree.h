...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    if( elements.empty() || val<elements.front() ){ return 0; }
    if( val > elements.back() ){ return elements.size(); }
    size_t idx = (elements.size()-1)/2;
    size_t prev_high = elements.size();
    size_t prev_low = 0;
    while( !((val<elements[idx]&&val>elements[idx-1])||val==elements[idx]) ){
        if( val < elements[idx] ){
            prev_high = idx;
            idx = (idx+prev_low)/2;
        } else {
