...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for( unsigned int x = 0; x < image.width(); x++ ){
    for( unsigned int y = 0; y < image.height(); y++ ){
      ...
      HSLAPixel* ptr = image.getPixel(x,y);
      (dist > 200) ? (ptr->l = 0) : (ptr->l *= (1-0.005*dist));
    }
  }
  return image;
}
...
PNG watermark(PNG firstImage, PNG secondImage) {
  ...
  for( unsigned int x = 0; x < firstImage.width(); x++ ){
    for( unsigned int y = 0; y < firstImage.height(); y++ ){
      HSLAPixel* ptr_second = secondImage.getPixel(x,y);
      if( ptr_second->l == 1 ){
        HSLAPixel* ptr_first = firstImage.getPixel(x,y);
        (ptr_first->l<=0.8) ? (ptr_first->l+=0.2) : (ptr_first->l=1);
      }
    }
  }
  return firstImage;
