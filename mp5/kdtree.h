...
class KDTree
{
  ...
  void swap(int idx_1, int idx_2, vector<Point<Dim>>& vector);
  void quickSelect(int left, int right, int dim, int k, vector<Point<Dim>>& vector);
  KDTreeNode* buildHelper(vector<Point<Dim>>& vector, int depth, int left, int right);
  void findNearestHelper(KDTreeNode* potential, const Point<Dim>& query,
                         Point<Dim>& currpoint, double& distbest, bool& flag, int depth) const;
  KDTreeNode* copy(KDTreeNode* lhs, const KDTreeNode* rhs);
