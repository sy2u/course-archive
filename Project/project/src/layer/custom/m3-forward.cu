...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    for (int i = 0; i < STREAM_NUM; i++){
        ...
        size_t curr_batch = Batch / STREAM_NUM;;
        if (i == STREAM_NUM-1) curr_batch = Batch - (STREAM_NUM-1) * curr_batch;
        size_t  offset_input = (size_t) sizeof(float)*Channel*Height*Width* curr_batch;
        size_t  offset_unrolled = (size_t) sizeof(float)*Channel*K*K*Height_out*Width_out* curr_batch;
        size_t  offset_output = (size_t) sizeof(float)*Height_out*Width_out*Map_out* curr_batch;

        // Copy: Host -> Device
        cudaMemcpyAsync((void*)(device_input+offset_input*i), host_input_temp+offset_input*i, offset_input, cudaMemcpyHostToDevice, stream[i]);
        ...
        // Copy: Device -> Host
        cudaMemcpyAsync((void*)(host_output_temp+offset_output*i), device_output+offset_output*i, offset_output, cudaMemcpyDeviceToHost, stream[i]);
    }
