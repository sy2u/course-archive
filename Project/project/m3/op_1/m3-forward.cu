    ...
    */
    ...
    if( h < Height_out && w < Width_out ){
        ...
        if ( K == 3 ) {
            ...
            out_3d(b, w_base + 0 * K + 1, w_unroll) = in_4d(b ,c, h + 0, w + 1);
            out_3d(b, w_base + 0 * K + 2, w_unroll) = in_4d(b ,c, h + 0, w + 2);
            out_3d(b, w_base + 0 * K + 3, w_unroll) = in_4d(b ,c, h + 0, w + 3);
            out_3d(b, w_base + 0 * K + 4, w_unroll) = in_4d(b ,c, h + 0, w + 4);
            out_3d(b, w_base + 0 * K + 5, w_unroll) = in_4d(b ,c, h + 0, w + 5);
            out_3d(b, w_base + 0 * K + 6, w_unroll) = in_4d(b ,c, h + 0, w + 6);
            out_3d(b, w_base + 1 * K + 0, w_unroll) = in_4d(b ,c, h + 1, w + 0);
            out_3d(b, w_base + 1 * K + 1, w_unroll) = in_4d(b ,c, h + 1, w + 1);
            out_3d(b, w_base + 1 * K + 2, w_unroll) = in_4d(b ,c, h + 1, w + 2);
            out_3d(b, w_base + 1 * K + 3, w_unroll) = in_4d(b ,c, h + 1, w + 3);
            out_3d(b, w_base + 1 * K + 4, w_unroll) = in_4d(b ,c, h + 1, w + 4);
            out_3d(b, w_base + 1 * K + 5, w_unroll) = in_4d(b ,c, h + 1, w + 5);
            out_3d(b, w_base + 1 * K + 6, w_unroll) = in_4d(b ,c, h + 1, w + 6);
            out_3d(b, w_base + 2 * K + 0, w_unroll) = in_4d(b ,c, h + 2, w + 0);
            out_3d(b, w_base + 2 * K + 1, w_unroll) = in_4d(b ,c, h + 2, w + 1);
            out_3d(b, w_base + 2 * K + 2, w_unroll) = in_4d(b ,c, h + 2, w + 2);  
            out_3d(b, w_base + 2 * K + 3, w_unroll) = in_4d(b ,c, h + 2, w + 3);
            out_3d(b, w_base + 2 * K + 4, w_unroll) = in_4d(b ,c, h + 2, w + 4);
            out_3d(b, w_base + 2 * K + 5, w_unroll) = in_4d(b ,c, h + 2, w + 5);
            out_3d(b, w_base + 2 * K + 6, w_unroll) = in_4d(b ,c, h + 2, w + 6);   
            out_3d(b, w_base + 3 * K + 0, w_unroll) = in_4d(b ,c, h + 3, w + 0);
            out_3d(b, w_base + 3 * K + 1, w_unroll) = in_4d(b ,c, h + 3, w + 1);
            out_3d(b, w_base + 3 * K + 2, w_unroll) = in_4d(b ,c, h + 3, w + 2);
            out_3d(b, w_base + 3 * K + 3, w_unroll) = in_4d(b ,c, h + 3, w + 3);
            out_3d(b, w_base + 3 * K + 4, w_unroll) = in_4d(b ,c, h + 3, w + 4);
            out_3d(b, w_base + 3 * K + 5, w_unroll) = in_4d(b ,c, h + 3, w + 5);
            out_3d(b, w_base + 3 * K + 6, w_unroll) = in_4d(b ,c, h + 3, w + 6);
            out_3d(b, w_base + 4 * K + 0, w_unroll) = in_4d(b ,c, h + 4, w + 0);
            out_3d(b, w_base + 4 * K + 1, w_unroll) = in_4d(b ,c, h + 4, w + 1);
            out_3d(b, w_base + 4 * K + 2, w_unroll) = in_4d(b ,c, h + 4, w + 2);
            out_3d(b, w_base + 4 * K + 3, w_unroll) = in_4d(b ,c, h + 4, w + 3);
            out_3d(b, w_base + 4 * K + 4, w_unroll) = in_4d(b ,c, h + 4, w + 4);
            out_3d(b, w_base + 4 * K + 5, w_unroll) = in_4d(b ,c, h + 4, w + 5);
            out_3d(b, w_base + 4 * K + 6, w_unroll) = in_4d(b ,c, h + 4, w + 6);
            out_3d(b, w_base + 5 * K + 0, w_unroll) = in_4d(b ,c, h + 5, w + 0);
            out_3d(b, w_base + 5 * K + 1, w_unroll) = in_4d(b ,c, h + 5, w + 1);
            out_3d(b, w_base + 5 * K + 2, w_unroll) = in_4d(b ,c, h + 5, w + 2); 
            out_3d(b, w_base + 5 * K + 3, w_unroll) = in_4d(b ,c, h + 5, w + 3);
            out_3d(b, w_base + 5 * K + 4, w_unroll) = in_4d(b ,c, h + 5, w + 4);
            out_3d(b, w_base + 5 * K + 5, w_unroll) = in_4d(b ,c, h + 5, w + 5);
            out_3d(b, w_base + 5 * K + 6, w_unroll) = in_4d(b ,c, h + 5, w + 6);    
            out_3d(b, w_base + 6 * K + 0, w_unroll) = in_4d(b ,c, h + 6, w + 0);
            out_3d(b, w_base + 6 * K + 1, w_unroll) = in_4d(b ,c, h + 6, w + 1);
            out_3d(b, w_base + 6 * K + 2, w_unroll) = in_4d(b ,c, h + 6, w + 2);   
            out_3d(b, w_base + 6 * K + 3, w_unroll) = in_4d(b ,c, h + 6, w + 3);
            out_3d(b, w_base + 6 * K + 4, w_unroll) = in_4d(b ,c, h + 6, w + 4);
            out_3d(b, w_base + 6 * K + 5, w_unroll) = in_4d(b ,c, h + 6, w + 5);
            out_3d(b, w_base + 6 * K + 6, w_unroll) = in_4d(b ,c, h + 6, w + 6);  
        } else {
            for( int p = 0; p < K; p++ ){
                for( int q = 0; q < K; q++ ){
