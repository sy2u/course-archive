...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    BTreeNode* new_right = child;
    BTreeNode* new_left = new BTreeNode(child->is_leaf, order);
    /* E.g.
     * | 3 | 6 | 8 |
    ...
    auto child_itr = parent->children.begin() + child_idx;
    ...
    /* TODO Your code goes here! */
    /* 1. creat new right node and copy mid child's key and value
     *     | 2 | 6 |
     *    /     \
     * | 1 |   | 3 | 6 | 8 |*/
    DataPair newElem = child->elements[mid_elem_idx];
    parent->elements.insert(elem_itr,newElem);
    /* 2, copy over the elements (and children) to the right of the child median 
     * into the new right node, and make sure the left node only has the elements
     * (and children) to the left of the child median.
     *     | 2 | 6 |
     *    /   /     \
     * | 1 | | 3 | | 8 |
     */
    new_left->elements.assign(child->elements.begin(),mid_elem_itr);
    new_right->elements.erase(child->elements.begin(),mid_elem_itr+1);
    if(!new_right->is_leaf){
        new_left->children.assign(child->children.begin(),mid_child_itr);
        new_right->children.erase(child->children.begin(),mid_child_itr);
    }
    parent->children.insert(child_itr,new_left);
}
