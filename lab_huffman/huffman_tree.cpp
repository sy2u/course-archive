...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    int num = singleQueue.size()+mergeQueue.size();
    while( num > 1 ){
        TreeNode* first = removeSmallest(singleQueue,mergeQueue);
        TreeNode* second = removeSmallest(singleQueue,mergeQueue);
        Frequency merged(first->freq.getFrequency()+second->freq.getFrequency());
        TreeNode* parentNode = new TreeNode(merged);
        parentNode->left = first;
        parentNode->right = second;
        mergeQueue.push(parentNode);
        num = singleQueue.size()+mergeQueue.size();
    }
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
        int direction = bfile.getNextBit();
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if( bfile.hasBits() ){
        if( bfile.getNextBit() ){
            ...
            TreeNode* leafNode = new TreeNode(leafFreq);
            return leafNode;
        } else {
            Frequency interFreq(0);
            TreeNode* interNode = new TreeNode(interFreq);
